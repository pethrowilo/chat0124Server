...
if (vgtype_ == network_type_data)
{
	...
	// **
	show_debug_message(
		string(ds_map_find_value(vgmap_, "type")) + string(date_current_datetime()) + 
		" (" + ds_map_find_value(vgmap_, "user") + ") [" +
		ds_map_find_value(vgmap_, "data") + "]"
	);
	var vdbg;
	vdbg = network_send_udp_raw(vgid_, vgip_, vgport_, vgbuffer_, vgsize_);
	show_debug_message("SENT: " + string(vdbg));
}